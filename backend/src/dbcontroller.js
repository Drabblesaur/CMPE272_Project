...
const userDataController = (fastify, options, done) => {
    ...
    // Create user data
    fastify.post('/userData', { schema: responseSchema }, async (request, reply) => {
        const { githubID, accessToken, refreshToken, profileData, data } = request.body;
        try {
            const newUserData = new userDataModel({ githubID, accessToken, refreshToken, profileData, data });
            const savedData = await newUserData.save();
            return reply.send({ message: 'User data created successfully!', data: savedData });
        } catch (error) {
            return reply.status(500).send({ message: 'Error creating user data', error: error.message });
        }
    });

    // Get all user data
    fastify.get('/userData', { schema: responseSchema }, async (request, reply) => {
        try {
            ...
            return reply.send({ message: 'User data retrieved successfully!', data: allUserData });
        } catch (error) {
            console.error('Error retrieving user data:', error); // Log any errors
            return reply.status(500).send({ message: 'Error retrieving user data', error: error.message });
        }
    });

    // Get user data by githubID
    fastify.get('/userData/:githubID', { schema: responseSchema }, async (request, reply) => {
    ...
    fastify.put('/addUserData/:githubID', { schema: responseSchema }, async (request, reply) => {
        ...
        try {
            ...
            return reply.send({ message: 'User data updated successfully!', data: updatedUserData.data });
        } catch (error) {
            // Handle any errors that occur during the update
            return reply.status(500).send({ message: 'Error updating user data', error: error.message });
        }
    });

    // Delete user data by ID
    fastify.delete('/userData/:id', { schema: responseSchema }, async (request, reply) => {
        ...
        try {
            ...
            return reply.status(500).send({ message: 'Error deleting user data', error: error.message });
        }
    });
